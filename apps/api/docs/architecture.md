...
`src/app.ts` is the composition root. It configures Fastify, CORS, the central error handler, `/health`, OAuth/session auth, feature repositories, and feature route registration. Public auth paths are `/auth/google`, `/auth/google/callback`, `/auth/me`, and `/auth/logout`; workspace paths are scoped under `/organizations/:organizationId`, while `/providers` remains global. Infrastructure and privacy analytics/advertising/newsletter provider selections resolve selected provider IDs through the Airtable-backed provider catalog before upserting system providers.
