...
describe("organizations API", () => {
  ...
  describe("granular GET profile section endpoints", () => {
    it("returns empty default profiles when organization profile is unset", async () => {
      const app = await createTestApp();
      
      const profileRes = await app.inject({
        method: "GET",
        url: "/organizations/org-test/profile",
      });
      expect(profileRes.statusCode).toBe(200);
      expect(profileRes.json().companyName).toBe("");
      ...
      expect(servicesRes.json()).toHaveLength(1);
      expect(servicesRes.json()[0].serviceName).toBeNull();

      const dataRes = await app.inject({
        method: "GET",
        url: "/organizations/org-test/data",
      });
      expect(dataRes.statusCode).toBe(200);
      expect(dataRes.json().dataTypesStored).toEqual([]);

      const privacyRes = await app.inject({
        method: "GET",
        url: "/organizations/org-test/privacy",
      });
      expect(privacyRes.statusCode).toBe(200);
      expect(privacyRes.json().crossBorderTransfers).toBeNull();

      const infraRes = await app.inject({
        method: "GET",
        url: "/organizations/org-test/infrastructure",
      });
      expect(infraRes.statusCode).toBe(200);
      expect(infraRes.json().organizationProviders).toEqual([]);

      const securityRes = await app.inject({
        method: "GET",
        url: "/organizations/org-test/security",
      });
      expect(securityRes.statusCode).toBe(200);
      expect(securityRes.json().penetrationTestingStrategy).toBeNull();

      const accessRes = await app.inject({
        method: "GET",
        url: "/organizations/org-test/access",
      });
      expect(accessRes.statusCode).toBe(200);
      expect(accessRes.json().mfaRequired).toBeNull();
    });

    it("returns the exact saved sections after they are updated", async () => {
      const app = await createTestApp();
      ...
      expect(profileRes.statusCode).toBe(200);
      expect(profileRes.json().companyName).toBe("Acme AI");
      ...
      expect(servicesRes.statusCode).toBe(200);
      expect(servicesRes.json()).toHaveLength(1);
      expect(servicesRes.json()[0].serviceName).toBe("Acme AI Platform");
      ...
      expect(dataRes.statusCode).toBe(200);
      expect(dataRes.json().dataTypesStored).toHaveLength(2);
      expect(dataRes.json().dataTypesStored[0].name).toBe("Customer account data");
      ...
      expect(privacyRes.statusCode).toBe(200);
      expect(privacyRes.json().crossBorderTransfers).toBe(true);
      ...
      expect(infraRes.statusCode).toBe(200);
      expect(infraRes.json().organizationProviders).toHaveLength(1);
      expect(infraRes.json().organizationProviders[0].providerId).toBe("prov-github");
      ...
      expect(securityRes.statusCode).toBe(200);
      expect(securityRes.json().penetrationTestingStrategy).toBe("external");
      ...
      expect(accessRes.statusCode).toBe(200);
      expect(accessRes.json().mfaRequired).toBe(true);
    });
  });
});
