...
The Plyco tool can resolve provider details from a provider URL with `pnpm plyco providers lookup --url <url>`. It can also import the resolved provider into Airtable with `pnpm plyco providers import --url <url>`, upserting the matching provider organization and provider rows. Lookup passes only the current Airtable `Provider Categories.Code` values and provider system type code IDs into the LLM prompt, constrains the LLM response to those IDs, and does not create or update app records; import writes only to Airtable and fails if the resolved provider category does not already exist in the linked provider category table.
