...
an empty rule framework list denotes a baseline check. Tagged rules enter scope
when at least one framework matches the organization's selected compliance
goals; `appliesWhen` is reserved for contextual applicability within that scope.
...
The same response exposes every in-scope rule as failing, missing data, passing,
contextually not applicable, or suppressed. Organization rule suppressions are
...
rule suppressions are persisted in `organization_rule_suppressions`.
